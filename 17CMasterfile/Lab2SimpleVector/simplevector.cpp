...
{
    T *newT = new T[arraySize + 1];
    if (!newT)
        memError();
        newT[0] = val;
        
    for (int i = 0; i < arraySize; i++) {
        newT[i + 1] = aptr[i];   //everything has to move forward to get new value in the FRONT of array.
    }
...
void SimpleVector<T>::pshBack(T val)
{
   T *newT = new T[arraySize + 1];
    if (!newT)
        memError();     //throw error if new array cannot be allocated in memory
...
T SimpleVector<T>::popFrnt()
{
   T frontValue = aptr[0];
    ...
    delete[] aptr;
    aptr = newT;
    arraySize--;        //for both front and back the new array size will decrement by one each function call.
    return frontValue;
}
