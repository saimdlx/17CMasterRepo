...
Table::Table(const Table &a){
    ...
    for (int i = 0 ; i < szCol ; i++) {             //Traverse through number of columns.
        columns[i] = new RowAray(*a.columns[i]);    //Declare new RowAray with data from parsed table objects column element at index i (RowAray data).
    }
...
Table::~Table(){
    for (int i = 0; i < szCol ; i++) {
        delete columns[i];                          //Delete data within columns (RowAray objects)
    }
    delete[] columns;                               //Delete columns array itself!
}

int Table::getData(int row,int col)const{
    if (col < szCol && col >= 0) {                  //Change/Return information as long as you are within reasonable bounds. Greater or equal to zero, but less than the amount of designated columns.
...
PlusTab PlusTab::operator+(const PlusTab &a){
    PlusTab mutate(szRow, szCol);
    for (int i = 0 ; i < szRow; i++) {                      //Traversing through each row and column in the table, adding the referenced table ->this with parsed table a by each row and column i,j, then setting the
                                                            //data at that row col index to the newly computed value;
        for (int j = 0 ; j < szCol ; j++) {

            int val = this->getData(i,j) + a.getData(i,j);
            mutate.setData(i,j,val);

        }
    }
    

    return mutate;                                          //Return the NEW array, not the old one (idiot).
}
