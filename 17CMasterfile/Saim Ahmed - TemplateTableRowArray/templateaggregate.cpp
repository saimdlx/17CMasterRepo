...
Table<T>::Table(const Table &a){
    ...
    for (int i = 0 ; i < szCol ; i++){
        columns[i] = new RowAray<T>(*a.columns[i]); //Copy of table 1, so just fetch values from parsed reference table.
    }
...
Table<T> Table<T>::operator+(const Table<T> &a){
    //Student to correct/complete
    Table<T> mutate(szRow, szCol);
    for (int i = 0 ; i < szRow; i++) {
        
        for (int j = 0 ; j < szCol ; j++) {
            
            int repVal = this->getData(i,j) + a.getData(i,j);
            mutate.setData(i,j,repVal);
