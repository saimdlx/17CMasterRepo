...
void prntAry(int **array,int *col,int *indx,int rows){
    for(int i=0;i<rows;i++){
        for(int j=0;j<col[indx[i]] ;j++){   //specific formatting using the size of col at index indx[i]
            cout<<array[indx[i]][j]<<" ";   //access and print array at structure element indx i col j. 
        }
        cout<<endl;
    }
    cout<<endl;
