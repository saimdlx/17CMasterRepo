...
int **fillAry(int *col,int rows){
    ...
    for(int i=0;i<rows;i++){
        array[i]=new int[rows]; //initialize arrays within array of pointers with size rows
    }
    for(int i=0;i<rows;i++){                                    //traverse i through rows (constant size)
        for(int j=0;j<col[i];j++){                              //this is modified to traverse only until j equals size of col at outer-index i.
